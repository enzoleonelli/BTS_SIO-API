const express = require("express");
const bodyParser = require("body-parser");
const cors = require("cors");

const app = express();

app.use(cors());

// analyser les requêtes de type de contenu - application / json
app.use(bodyParser.json());

// analyser les requêtes de type de contenu - application / x-www-form-urlencoded
app.use(bodyParser.urlencoded({ extended: true }));

/*const db = require("./app/models");
db.sequelize.sync();
const Role = db.role;*/

// simple route
app.get("/", (req, res) => {
  res.json({ message: "OK" });
});

//Route ordonance
require("./app/routes/ordonance.routes")(app);
//Route medicament
require("./app/routes/medoc.routes")(app);
//Route prise medicament
require("./app/routes/priseMedoc.routes")(app);
//Route authentification 
require('./app/routes/auth.routes')(app);
//Route utilisateur
require('./app/routes/user.routes')(app);

// définit le port, écouter les demandes
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`Server is running on port ${PORT}.`);
});

function initial() {
  Role.create({
    id: 1,
    name: "user"
  }); 
  Role.create({
    id: 2,
    name: "admin"
  });
}
